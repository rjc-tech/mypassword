...
package jp.co.rjc.mypassword.plugin;
...
public class SQLitePlugin extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args, CallbackContext cbc) {
		try {
			...
			if (action.equals("open")) {
				...
				JSONArray jparams = (args.length() < 3) ? null : args
						.getJSONArray(2);
				...
				Cursor myCursor = this.getDatabase(dbName).rawQuery(query,
						params);

				String result = this.getRowsResultFromQuery(myCursor)
						.getJSONArray("rows").toString();

				this.sendJavascriptCB("window.SQLitePluginCallback.p1('" + id
						+ "', " + result + ");");
				...
				if (action.equals("backgroundExecuteSqlBatch"))
					this.executeSqlBatchInBackground(dbName, queries,
							jsonparams, queryIDs, cbc);
				else
					this.executeSqlBatch(dbName, queries, jsonparams, queryIDs,
							cbc);
			}
	...
	private void openDatabase(String dbname, String password) {
		...
		File dbfile = this.cordova.getActivity()
	...
	private boolean deleteDatabase(String dbname) {
		...
		// Use try & catch just in case android.os.Build.VERSION.SDK_INT >= 16
		// was lying:
		try {
			...
			Log.v("executeSqlBatch",
					"deleteDatabase(): Error=" + ex.getMessage());
			ex.printStackTrace();
		}
	...
	/**
	 * Executes a batch request IN BACKGROUND THREAD and sends the results via
	 * sendJavascriptCB().
	 *
	 * @param dbName
	 *            The name of the database.
	...
	private void executeSqlBatchInBackground(final String dbName,
		...
		this.cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				synchronized (myself) {
					myself.executeSqlBatch(dbName, queryarr, jsonparams,
							queryIDs, cbc);
				}
			}
		});
	 ...
	 *
	 */
	private void executeSqlBatch(String dbname, String[] queryarr,
			JSONArray[] jsonparams, String[] queryIDs, CallbackContext cbc) {
		SQLiteDatabase mydb = this.getDatabase(dbname);
		...
		for (int i = 0; i < len; i++) {
			...
			try {
				...
				if (android.os.Build.VERSION.SDK_INT >= 11
						&& (query.toLowerCase().startsWith("update") || query
								.toLowerCase().startsWith("delete"))) {
					SQLiteStatement myStatement = mydb.compileStatement(query);

					if (jsonparams != null) {
						for (int j = 0; j < jsonparams[i].length(); j++) {
							if (jsonparams[i].get(j) instanceof Float
									|| jsonparams[i].get(j) instanceof Double) {
								myStatement.bindDouble(j + 1,
										jsonparams[i].getDouble(j));
							} else if (jsonparams[i].get(j) instanceof Number) {
								myStatement.bindLong(j + 1,
										jsonparams[i].getLong(j));
							} else if (jsonparams[i].isNull(j)) {
								myStatement.bindNull(j + 1);
							} else {
								myStatement.bindString(j + 1,
										jsonparams[i].getString(j));
							}
						}
					}
					...
					// android.os.Build.VERSION.SDK_INT >= 11 is lying:
					try {
						...
						Log.v("executeSqlBatch",
								"SQLiteStatement.executeUpdateDelete(): Error="
										+ errorMessage);
						needRawQuery = false;
					} catch (Exception ex) {
				...
				if (query.toLowerCase().startsWith("insert")
					...
					try {
						...
						errorMessage = ex.getMessage();
						Log.v("executeSqlBatch",
								"SQLiteDatabase.executeInsert(): Error="
										+ errorMessage);
					}
				...
				if (query.toLowerCase().startsWith("begin")) {
					...
					try {
						...
						errorMessage = ex.getMessage();
						Log.v("executeSqlBatch",
								"SQLiteDatabase.beginTransaction(): Error="
										+ errorMessage);
					}
				}
				...
				if (query.toLowerCase().startsWith("commit")) {
					...
					try {
						...
						errorMessage = ex.getMessage();
						Log.v("executeSqlBatch",
								"SQLiteDatabase.setTransactionSuccessful/endTransaction(): Error="
										+ errorMessage);
					}
				}
				...
				if (query.toLowerCase().startsWith("rollback")) {
					...
					try {
						...
						errorMessage = ex.getMessage();
						Log.v("executeSqlBatch",
								"SQLiteDatabase.endTransaction(): Error="
										+ errorMessage);
					}
				}
			...
			do {
				...
				try {
					for (int i = 0; i < colCount; ++i) {
						...
						// NOTE: this code should be safe to RUN with old
						// Android SDK.
						// To BUILD with old Android SDK remove lines from HERE:
						// {{
						if (android.os.Build.VERSION.SDK_INT >= 11) {
